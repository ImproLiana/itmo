...
public class ExecuteScriptCommand implements ICommand {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(canonicalPath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (complexCommands.contains(command)) {
                    response = processComplexCommand(reader, command, parts, manager);
                } else {
                    ...
                    if (cmd == null) {
                        ...
                        response = cmd.execute(arg, manager);
                    }
                }
    ...
    private CommandResponse processComplexCommand(BufferedReader reader, String command, String[] parts, ServerCollectionManager manager)
            throws IOException, BadArgumentException {
        switch (command) {
            case "add": {
                Dragon dragon = readDragonFromScript(reader, GetUniqueId.generateId(manager));
            ...
            case "update_id": {
                ...
                manager.getCollection().removeIf(d -> Objects.equals(d.getId(), userId));
                Dragon updatedDragon = readDragonFromScript(reader, userId);
                manager.addDragon(updatedDragon);
            ...
            case "execute_script": {
                ...
                CommandResponse nestedResponse = this.execute(nestedPath, manager);
    ...
    private Dragon readDragonFromScript(BufferedReader reader, Long id) throws IOException {
